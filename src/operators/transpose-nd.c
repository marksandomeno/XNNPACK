  ...
  switch (normalized_dims) {
    ...
    case 2:
      if (variable_size_ukernel) {
        transpose_op->compute.type = xnn_parallelization_type_2d_tile_1d;
        transpose_op->compute.task_2d_tile_1d = (pthreadpool_task_2d_tile_1d_t) xnn_compute_transposev_2d;
      } else {
    ...
    case 3:
      if (variable_size_ukernel) {
        transpose_op->compute.type = xnn_parallelization_type_3d_tile_1d;
        transpose_op->compute.task_3d_tile_1d = (pthreadpool_task_3d_tile_1d_t) xnn_compute_transposev_3d;
      } else {
    ...
    case 4:
      if (variable_size_ukernel) {
        transpose_op->compute.type = xnn_parallelization_type_4d_tile_1d;
        transpose_op->compute.task_4d_tile_1d = (pthreadpool_task_4d_tile_1d_t) xnn_compute_transposev_4d;
      } else {
    ...
    case 5:
      if (variable_size_ukernel) {
        transpose_op->compute.type = xnn_parallelization_type_5d_tile_1d;
        transpose_op->compute.task_5d_tile_1d = (pthreadpool_task_5d_tile_1d_t) xnn_compute_transposev_5d;
      } else {
    ...
    case 6:
      if (variable_size_ukernel) {
        transpose_op->compute.type = xnn_parallelization_type_6d_tile_1d;
    ...
    pthreadpool_t threadpool)
{
  ...
  const size_t input_shape[6] = {batch_size, block_size, block_size, depth_to_space_op->channels, input_height, input_width};
  const size_t perm[6] = {0, 4, 1, 5, 2, 3};
  const size_t input_stride[6] = {
    depth_to_space_op->input_pixel_stride * input_height * input_width,
    block_size * depth_to_space_op->channels * input_height * input_width,
    depth_to_space_op->channels * input_height * input_width,
  ...
  return setup_transpose(
    ...
    input_shape,
    perm,
    ((depth_to_space_op->input_pixel_stride == block_size * block_size * depth_to_space_op->channels) ? NULL : input_stride),
    ((depth_to_space_op->output_pixel_stride == depth_to_space_op->channels) ? NULL : output_stride),
    sizeof(uint32_t));
}
    ...
    uint32_t element_size)
{
  ...
  const size_t input_shape[5] = {batch_size * input_height, input_width, block_size, block_size, depth_to_space_op->channels};
  const size_t perm[5] = {0, 2, 1, 3, 4};
  const size_t input_stride[5] = {
    input_width * depth_to_space_op->input_pixel_stride,
    depth_to_space_op->input_pixel_stride,
    block_size * depth_to_space_op->channels,
    depth_to_space_op->channels,
    1};
