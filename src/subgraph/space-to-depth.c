  ...
  switch (node->compute_type) {
    ...
    case xnn_compute_type_fp16:
      status = xnn_create_space_to_depth_nhwc_x16(
          ...
          node->params.space_to_depth.block_size,
          node->flags,
...
{
  enum xnn_status status;
  if ((status = xnn_subgraph_check_xnnpack_initialized(xnn_node_type_space_to_depth)) != xnn_status_success) {
    return status;
  }

  if ((status = xnn_subgraph_check_input_node_id(xnn_node_type_space_to_depth, input_id, subgraph->num_values)) !=
      xnn_status_success) {
    return status;
  }

  const struct xnn_value* input_value = &subgraph->values[input_id];
  status = xnn_subgraph_check_input_type_dense(xnn_node_type_space_to_depth, input_id, input_value);
  if (status != xnn_status_success) {
    return status;
  }
  ...
  switch (input_value->datatype) {
    ...
    default:
      xnn_log_error(
        ...
        xnn_node_type_to_string(xnn_node_type_space_to_depth), input_id,
        xnn_datatype_to_string(input_value->datatype), input_value->datatype);
      return xnn_status_invalid_parameter;
  }

  status = xnn_subgraph_check_output_node_id(xnn_node_type_space_to_depth, output_id, subgraph->num_values);
  if (status != xnn_status_success) {
    return status;
  }

  const struct xnn_value* output_value = &subgraph->values[output_id];
  status = xnn_subgraph_check_output_type_dense(xnn_node_type_space_to_depth, output_id, output_value);
  if (status != xnn_status_success) {
    return status;
  }
  ...
  switch (output_value->datatype) {
    ...
    default:
      xnn_log_error(
        ...
        xnn_node_type_to_string(xnn_node_type_space_to_depth), output_id,
  ...
  status = xnn_subgraph_check_datatype_matches(
    xnn_node_type_space_to_depth, input_id, input_value, output_id, output_value);
  if (status != xnn_status_success) {
    return status;
  }
  ...
  if (compute_type == xnn_datatype_qint8 || compute_type == xnn_datatype_quint8) {
    if (input_value->quantization.zero_point != output_value->quantization.zero_point) {
      xnn_log_error(
        ...
        xnn_node_type_to_string(xnn_node_type_space_to_depth), input_id, output_id,
  ...
  if (block_size < 2) {
    xnn_log_error(
      ...
      xnn_node_type_to_string(xnn_node_type_space_to_depth), block_size);
    return xnn_status_invalid_parameter;
  }
  ...
  node->type = xnn_node_type_space_to_depth;
  node->compute_type = compute_type;
  node->num_inputs = 1;
  node->inputs[0] = input_id;
  node->num_outputs = 1;
  node->outputs[0] = output_id;
  node->params.space_to_depth.block_size = block_size;
  node->flags = flags;
